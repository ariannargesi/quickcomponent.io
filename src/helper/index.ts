...
export const addStyleInNode = (
  ...
  if (isTextNode(element))
    element = getParentNode(map, key);
  const style = element.props.style;
  if (!element.props.className)
    element.props.className = generateClassName(
      element.title
    );
  element.props.style = {
    ...style,
...
export const isHtmlTag = (value: string): boolean => {
  return (
    elementsList.filter((item) => item.tag === value)
      .length > 0
  );
};

export const isTextNode = (
  value: ComponentMember
): boolean => {
...
export const genereateElement = (
  ...
  if (name === 'button') {
    ...
    return {
      ...
      key: elementKey,
      children: [
        { title: 'text', text: 'Text', key: innerKey },
      ],
    };
  }
};
...
export const convertProps = (propsList) => {
  const isJsProp =
    propsList[0].type.startsWith('PropTypes');
