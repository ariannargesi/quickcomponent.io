...
import styles from '@app/components/ComponentView/styles.module.sass';
import { useDispatch, useSelector } from 'react-redux';

const Input = (props: { username: string }) => {
  const defaultText = useSelector((state: RootState) => {
    return findNodeInTree(state.map, state.selectedKey)
      .text;
  });
...
export const arrayToComponent = (
  ...
  map.forEach((element) => {
    ...
    if (inputKey === element.key) {
    ...
    } else if (element.text || element.title === 'img') {
      // This is for rendering text and the elements that does not contain any childs
      const title = element.title;
      ...
      if (element.title === 'img') {
        ...
        component.push(
          React.createElement(title, props, text)
        );
      }
    } else {
